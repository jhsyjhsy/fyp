...
# --------- CONFIG ----------
BASE_MODEL = "mistralai/Mistral-7B-Instruct-v0.3"  # matches your finetune
ADAPTER_REPO = "jhsyjhsy/mistral_finetune_v1"      # your HF repo
CACHE_DIR = "/data/users/huisinyu/hf_cache"

INPUT_CSV = "CLEAR_dev_only_mistral_lay.csv"                   # or CLEAR_test_only.csv
OUTPUT_CSV = "CLEAR_dev_with_finetuned_outputs.csv"
...
for idx, row in tqdm(df.iterrows(), total=len(df), desc="Generating lay summaries"):
    ...
    with torch.no_grad():
        outputs = model.generate(
            **inputs,
            max_new_tokens=160,
            temperature=0.2,  # lower than base model for consistency
            top_p=0.9,
            do_sample=True,
        )
